...
namespace GeometricShapes
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($"{circle.Name} - Area: {circle.Area()}, Perimeter: {circle.Perimeter()}");

            Square square = new Square(4);
            Console.WriteLine($"{square.Name} - Area: {square.Area()}, Perimeter: {square.Perimeter()}");

            EquilateralTriangle triangle = new EquilateralTriangle(3);
            Console.WriteLine($"{triangle.Name} - Area: {triangle.Area()}, Perimeter: {triangle.Perimeter()}");
        }
    }
}